...
// Create a Schema object for the Book Store model
...
// Mount the router middleware at the '/api/book stores' path.
// All routes defined on this router will be prefixed with '/api/bookstores'.
app.use('/api/bookstores', router);
// 1. Route to get all book stores from the database.
// Handles GET requests to '/api/book stores/'.
// http://localhost:3000/api/bookstores
router.route("/")
    .get((req, res) => {
        console.log("Fetching all book stores..."); // Log the request for debugging purposes.
        // Find all book store documents in the 'book stores' collection., 400=data was not found
        BookStore.find()
...
// 2. Route to get a specific book store by its ID.
// Handles GET requests to '/api/book stores/:id'.
router.route("/:id")
    .get((req, res) => {
...
// 3. Route to add a new book store to the database.
// Handles POST requests to '/api/book stores/add'.
router.route("/add")
    .post((req, res) => {
...
// 4. Route to update an existing book store by its ID.
// Handles PUT requests to '/api/book stores/update/:id'.
...
// 5. Route to delete a book store by its ID.
// Handles DELETE requests to '/api/book stores/delete/:id'.
...
// Simple Rest API setup data
// This is derived from const BookStore = mongoose.model("BookStore", bookstoreSchema);
// Model Name: Enter table name-BookStore
// from ...mongodb.net/Book?retryWrites=true...
// MongoDB database name=database name-Book
// Model Attributes: table attributes
// MongoDB URI: cluster0.sfmiwfn.mongodb.net
// mongoose.connect('mongodb+srv://woof2:Jayden1950@cluster0.sfmiwfn.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0/Book'